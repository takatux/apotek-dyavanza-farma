...
<script type="text/javascript">
    ...
    function fetch_data(search='')
    {
        $('.table-obat').DataTable({
            ...
            url:"{{ route('admin-getDataPemesan') }}",
        ...
        $("body").on("click", ".modal-tab-acc", function() {
            var judulid = $(this).attr('data-id');
            swal({
                title: "Yakin?",
                text: "Pesanan ini akan di ACC ?",
                icon: "warning",
                buttons: ["Batal", "OK"],
                dangerMode: true,
                })
                .then((willDelete) => {
                if (willDelete) {
                    window.location = "/admin/acc/"+judulid+"" 
                    swal("Data berhasil di acc", {
                    icon: "success",
                    });
                } else {
